...
def train(model, graph, labels, train_idx, optimizer, use_labels, forw_time, loss_time, back_time):
    model.train()
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running):
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        loss, pred = train(model, graph, labels, train_idx, optimizer, args.use_labels, forw_time, loss_time, back_time)
        ...
        if epoch % args.log_every == 0:
            gpu_mem = th.cuda.max_memory_allocated()/1024/1024 if th.cuda.is_available() else 0
            print(
                f"R {n_running}/{args.n_runs} E {epoch}/{args.n_epochs} | Time e/forw/loss/back/eval "
                f"{np.mean(total_time[2:]):.5f}/{np.mean(forw_time[2:]):.5f}/{np.mean(loss_time[2:]):.5f}/{np.mean(back_time[2:]):.5f}/{np.mean(eval_time[2:]):.5f} | "
                #f"Loss {loss.item():.4f} | Acc {acc:.4f} | Train/Val/Test loss {train_loss:.4f}/{val_loss:.4f}/{test_loss:.4f} | "
...
def main():
    ...
    data_root = os.path.join(home_root, 'datasets/')
    tic = time.time()
    ...
    # add reverse edges-------------------------------------------
    print(f"Total edges before adding reverse edges {graph.number_of_edges()}")
    tic = time.time()
